...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_padding_mask, output_attentions=False, enc_attn=None):
        """
        Args:
...
class BartEncoder(nn.Module):
    ...
    def forward(
        ...
        for encoder_layer in self.layers:
            ...
            # print('mod bart 378', encoder_layer, x, attn)
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        #     print('key encoder_hidden_states', encoder_hidden_states.shape)
        #     print('mode bart 421 enc', encoder_attn_mask.shape)
        # else:
        #     print(encoder_attn_mask)
        residual = x
...
class BartDecoder(nn.Module):
    ...
    def forward(
        ...
        **unused,
        ...
        # print('mod bart 590', encoder_hidden_states.shape)
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            # if decoder_padding_mask is not None:
            #     print('mod bart 616', decoder_causal_mask.shape)
            # print('mod bart 614', layer_self_attn.shape)
            if use_cache:
                next_decoder_cache.append(layer_past.copy())

            if output_attentions:
                all_self_attns += (layer_self_attn,)
        # print('mod bart 624', x.shape, x)

        if self.layer_norm:  # if config.add_final_layer_norm (mBART)
            x = self.layer_norm(x)
        # print('mod bart 628', x.shape, x)
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # print('mode bart 679 query', query.size())
        ...
        #     print('mod bart 719', key_padding_mask.shape)
        #     print('bsz, src_len', [bsz, src_len])
        ...
        if key_padding_mask is not None:  # don't attend to padding symbols
            attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len)
            if enc_attn is not None:
                reshaped = enc_attn.unsqueeze(1)
            else:
                if len(key_padding_mask.shape)==3:
                    reshaped = key_padding_mask.unsqueeze(1)
                else:
...
class LearnedPositionalEmbedding(nn.Embedding):
    ...
    def __init__(self, num_embeddings: int, embedding_dim: int, padding_idx: int, tag_num=0):
    ...
    def forward(self, input_ids, use_cache=False, src_pos=None):
        ...
        if src_pos is not None:
            ...
            # print('mode bart 865 用LearnedPositionalEmbedding')
            if not self.tag_num:
                positions = torch.arange(seq_len, dtype=torch.long, device=self.weight.device)
            else:
                ones = torch.ones(self.tag_num+1, dtype=torch.long, device=self.weight.device)
                ones[0] = 0
                positions = torch.arange(seq_len-self.tag_num-1, dtype=torch.long, device=self.weight.device)+2
                positions = torch.cat((ones, positions))
        # print('mod bart 895', positions + self.offset)
        # print('mod bart 896', super().forward(positions + self.offset).shape)
...
class SinusoidalPositionalEmbedding(nn.Embedding):
    ...
    def __init__(self, num_positions, embedding_dim, padding_idx=None, tag_num=0):
        self.offset = 2
        super().__init__(num_positions+self.offset, embedding_dim)
        self.tag_num = tag_num
